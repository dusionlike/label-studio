  ...
  const handleImport = (): void => {
    try {
      ...
      if (Array.isArray(parsedData)) {
        ...
        throw new Error("格式无效：需要传入快捷键数组，或包含 hotkeys 字段的对象");
      }

      // Validate it's not empty
      if (hotkeys.length === 0) {
        throw new Error("导入数据中没有找到快捷键");
      }
      ...
      hotkeys.forEach((hotkey: unknown, index: number) => {
        try {
          ...
          throw new Error(`索引 ${index} 的快捷键有误：${errorMessage}`);
        }
      });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px] bg-neutral-surface">
        <DialogHeader>
          ...
          <DialogDescription>
            请在下方粘贴你导出的快捷键 JSON。导入后会覆盖当前快捷键，请确认 JSON 中包含带有必填字段的快捷键对象数组。
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <textarea
            ...
            placeholder='[{"id": 1, "section": "annotation", "element": "annotation:submit", "label": "提交标注", "key": "ctrl+enter"}]'
            value={importText}
