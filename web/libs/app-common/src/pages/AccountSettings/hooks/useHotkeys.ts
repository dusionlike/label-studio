...
export const useHotkeys = () => {
  ...
  const loadHotkeysFromAPI = useCallback(async () => {
    try {
      ...
      if (toast) {
        toast.show({
          message: "无法从服务器加载自定义快捷键，已使用缓存设置",
  ...
  const saveHotkeysToAPI = useCallback(
    async (currentHotkeys: Hotkey[], currentSettings: HotkeySettings): Promise<SaveResult> => {
      ...
      try {
        ...
        const operation = isReset ? "重置" : "保存";
        console.error(`Error ${operation} hotkeys:`, error);

        // Provide more specific error messages
        let errorMessage = `快捷键${isReset ? "重置" : "保存"}失败`;
  ...
  const handleResetToDefaults = useCallback(() => {
    confirm({
      title: "确认恢复默认快捷键？",
      body: "你确定要将全部快捷键和相关设置恢复为默认值吗？此操作无法撤销。",
      okText: "恢复默认",
      buttonLook: "negative",
      style: { width: 500 },
      onOk: async () => {
        ...
        try {
          ...
          if (result.ok) {
            ...
            if (toast) {
              toast.show({
                message: `恢复默认保存失败：${result.error || "未知错误"}`,
                type: ToastType.error,
              });
            }
          }
