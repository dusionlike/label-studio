      ...
      <header className="mb-base">
        ...
        {webhooks.length > 0 && (
          <Typography size="small" className="text-neutral-content-subtler">
            使用 Webhook 配置订阅特定事件的集成。当事件触发时，系统会向配置好的 webhook URL 发送 HTTP POST 请求。
          </Typography>
        )}
      </header>
        ...
        {webhooks.length === 0 ? (
          <SimpleCard title="" className="bg-primary-background border-primary-border-subtler p-base">
            <EmptyState
              ...
              description="使用 Webhook 配置订阅特定事件的集成。当事件触发时，Label Studio 会向配置好的 webhook URL 发送 HTTP POST 请求。"
              actions={
                canChangeWebhooks ? (
